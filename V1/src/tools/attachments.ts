...
export const registerAttachmentTools: RegisterFn = (server, ctx) => {
  server.registerTool(
    "list_attachments",
    {
  ...
  server.registerTool(
    ...
    wrap(ctx, "upload_attachment", async ({ component, recordId, field, files }) => {
      const comp = await ctx.metadata.resolveComponent(component);
      const f = await ctx.metadata.resolveField(comp.Id, field);
      return ok(await ctx.api.updateRecordAttachments(comp.Id, recordId, f.Id,
        files.map((x) => ({ FileName: x.fileName, FileData: x.fileDataBase64 }))));
    }),
  );
