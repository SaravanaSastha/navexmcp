...
import type { Credentials, SessionManager } from "../services/session-manager.js";

export interface AuthedRequest extends Request {
  navexCredentials?: Credentials;
 ...
 */
export function createAuthMiddleware(config: Config, sessions: SessionManager) {
  const apiKeysEnabled =
    config.apiKeys.length > 0 && !!config.navexServiceUsername && !!config.navexServicePassword;
