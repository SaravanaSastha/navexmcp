...
export class NavexClient {
    ...
    static classifyLoginRefusal(body, baseUrl) {
        const t = body.trim();
        ...
        if (t === "")
            return "NAVEX returned an empty body.";
        return `Unexpected login response starting with: "${t.slice(0, 80)}"`;
    }
    lastAuthFailureAt = 0;
    lastAuthFailureMsg = "";
    async attemptLogin(format) {
        ...
        const res = await this.fetchFn(`${this.opts.baseUrl}/SecurityService/Login`, {
            ...
            body: format === "json"
                ...
                : `<Login><username>${xmlEscape(this.opts.username)}</username><password>${xmlEscape(this.opts.password)}</password>${this.opts.ldapSettingsId !== undefined ? `<ldapSettingsId>${this.opts.ldapSettingsId}</ldapSettingsId>` : ""}</Login>`,
        });
    ...
    async login() {
        ...
        if (Date.now() - this.lastAuthFailureAt < 60_000) {
            throw new NavexError(`${this.lastAuthFailureMsg} (Login attempts paused for 60s to protect the account from lockout.)`, "AUTH_FAILED");
        }
        this.loginPromise = (async () => {
            ...
            if (!result.cookie)
                throw new NavexError("NAVEX login returned no session cookie.", "AUTH_FAILED");
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    if (!res.ok)
                        throw fromHttpStatus(res.status, path);
...
//# sourceMappingURL=navex-client.js.map