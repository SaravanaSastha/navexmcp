 ...
 */
export function createApiRouter(registry) {
    const router = Router();
    function handle(op, fn) {
        return async (req, res) => {
            ...
            try {
                ...
                const status = err instanceof NavexError
    ...
    const ctxOf = (req) => registry.getContext(req.navexCredentials);
    /* ---- Metadata ---- */
    ...
    /* ---- Workflow ---- */
    router.get("/workflows", handle("getWorkflows", async (req) => ctxOf(req).api.getWorkflows(String(req.query.componentAlias ?? ""))));
    ...
    router.post("/assessments/issue", handle("issueAssessment", async (req) => ctxOf(req).api.issueAssessment(z.record(z.unknown()).parse(req.body ?? {}))));
    /* ---- Users & groups ---- */
    ...
    router.post("/users/search", handle("getUsers", async (req) => {
        ...
        return ctxOf(req).api.getUsers(body);
    }));
