...
const FILTER_IDS = { EqualTo: 5, NotEqualTo: 6, ContainsAny: 10002 };
function toNavexUserFilters(filters) {
...
export const registerSecurityTools = (server, ctx) => {
    /* ---- Users ---- */
    server.registerTool("get_user", { title: "Get user", description: "Returns all fields for a NAVEX IRM user by ID.", inputSchema: { userId: z.number().int().min(1) } }, wrap(ctx, "get_user", async ({ userId }) => ok(await ctx.api.getUser(userId))));
    ...
    }, wrap(ctx, "list_users", async ({ pageIndex, pageSize, filters }) => ok(await ctx.api.getUsers({ pageIndex, pageSize, filters: toNavexUserFilters(filters) }))));
    server.registerTool("get_user_count", {
        title: "Count users",
        description: "Returns the count of users matching optional filters.",
        inputSchema: { filters: z.array(userFilter).optional() },
    }, wrap(ctx, "get_user_count", async ({ filters }) => ok({ count: await ctx.api.getUserCount({ filters: toNavexUserFilters(filters) }) })));
    ...
    /* ---- Groups ---- */
    server.registerTool("get_group", { title: "Get group", description: "Returns all fields for a NAVEX IRM group by ID, including members.", inputSchema: { groupId: z.number().int().min(1) } }, wrap(ctx, "get_group", async ({ groupId }) => ok(await ctx.api.getGroup(groupId))));
    server.registerTool("list_groups", { title: "List groups", description: "Lists NAVEX IRM groups (ID and Name).", inputSchema: { ...paging } }, wrap(ctx, "list_groups", async ({ pageIndex, pageSize }) => ok(await ctx.api.getGroups({ pageIndex, pageSize }))));
