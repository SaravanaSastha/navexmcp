...
export const registerMetadataTools = (server, ctx) => {
    ...
    server.registerTool("get_field", {
        ...
        if (component === undefined)
            throw new Error("component is required when field is a name");
        const comp = await ctx.metadata.resolveComponent(component);
        return ok(await ctx.metadata.resolveField(comp.Id, field));
    }));
    server.registerTool("get_lookup_options", {
    ...
    }, wrap(ctx, "get_lookup_options", async ({ fieldId, recordId, pageIndex, pageSize }) => ok(await ctx.api.getAvailableLookupRecords({ fieldId, pageIndex, pageSize, recordId }))));
};
