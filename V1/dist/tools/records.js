...
import { componentRef, fieldRef, filterSchema, fieldsRecord, paging, confirmFlag, requireConfirm, toNavexValue } from "../schemas.js";
import { FILTER_TYPES } from "../services/navex-api.js";
async function buildFilters(ctx, componentId, filters) {
    if (!filters?.length)
        return undefined;
    return Promise.all(filters.map(async (f) => {
        const fieldPath = f.fieldPath ?? [(await ctx.metadata.resolveField(componentId, f.field)).Id];
        const item = { FieldPath: fieldPath, FilterType: FILTER_TYPES[f.filterType] };
        if (f.value !== undefined)
            item.Value = f.value;
        return item;
    }));
}
async function buildFieldValues(ctx, componentId, fields) {
    const entries = Object.entries(fields);
    return Promise.all(entries.map(async ([ref, value]) => {
        const field = await ctx.metadata.resolveField(componentId, ref);
        return { key: field.Id, value: toNavexValue(value) };
    }));
}
export const registerRecordTools = (server, ctx) => {
    server.registerTool("get_record", {
        ...
        if (detail)
            return ok(await ctx.api.getDetailRecord(comp.Id, recordId, embedRichTextImages));
    ...
    server.registerTool("search_records", {
        ...
        description: "Searches records in a component with filters and paging. Filter example: {field:'Status', filterType:'EqualTo', value:'Open'}. Use get_fields first to learn field names.",
        inputSchema: {
            component: componentRef,
    ...
    server.registerTool("create_record", {
        title: "Create record",
        description: "Creates a record. 'fields' maps field name (or ID) to value: string, number, boolean, null, {id} for 1:1 lookups, [{id},...] for 1:many lookups. NOTE: the API does not enforce required fields — validate with get_fields first.",
